...
class RandomWalker:
    ...
    def node2vec_walk2(self, walk_length, start_node):
        ...
        def rejection_sample(inv_p, inv_q, nbrs_num):
            upper_bound = max(1.0, inv_p, inv_q)
            lower_bound = min(1.0, inv_p, inv_q)
            shatter = 0
...
class BiasedWalker:
    ...
    def simulate_walks(self, num_walks, walk_length, stay_prob=0.3, workers=1, verbose=0):
        ...
        results = Parallel(
            ...
            delayed(self._simulate_walks)(
                nodes, num, walk_length, stay_prob, layers_adj, layers_accept, layers_alias, gamma
            )
    ...
    def _simulate_walks(self, nodes, num_walks, walk_length, stay_prob, layers_adj, layers_accept, layers_alias, gamma):
        ...
        for _ in range(num_walks):
            ...
            for v in nodes:
                walks.append(
                    self._exec_random_walk(layers_adj, layers_accept, layers_alias, v, walk_length, gamma, stay_prob)
                )
        return walks
